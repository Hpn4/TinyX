...
public class RepoPostController {
  ...
  @APIResponses({
    @APIResponse(
        responseCode = "200",
        description = "OK, returns a list of posts matching the given IDs"),
  ...
  public Response queryPostsList(List<UUID> postIds) {
    if (postIds == null || postIds.stream().anyMatch(Objects::isNull)) {
      return Response.status(Response.Status.BAD_REQUEST)
          .entity("ID list or one of its IDs is null")
          .build();
    }
  ...
  @APIResponses({
    @APIResponse(
        responseCode = "200",
        description = "OK, returns the posts authored by the given ID"),
  ...
  public Response queryUserPosts(@PathParam("authorId") UUID authorId) {
    ...
    if (posts == null) {
      return Response.status(Response.Status.NOT_FOUND)
          .entity("No posts match the given authordId: " + authorId.toString())
          .build();
    }
  ...
  public Response queryPost(@PathParam("postId") UUID postId) {
    ...
    if (post == null) {
      return Response.status(Response.Status.NOT_FOUND)
          .entity("No post matches the given ID")
          .build();
    }
  ...
  public Response queryUser(@PathParam("userId") UUID userId) {
    ...
    if (user == null) {
      return Response.status(Response.Status.NOT_FOUND)
          .entity("No user matches the given ID")
          .build();
    }
  ...
  public Response queryMedia(@PathParam("mediaId") UUID mediaId) {
    ...
    if (media == null) {
      return Response.status(Response.Status.NOT_FOUND)
          .entity("No media matches the given ID")
          .build();
    }
