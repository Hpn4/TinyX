...
public class RedisStreamUserRelationSocial extends RedisStreamReader<UserRelationsQuery> {
    ...
    public RedisStreamUserRelationSocial(){super();}
    ...
    public void process(List<UserRelationsQuery> data)
    {
        for(var i = 0; i<data.size();i++)
        {
            ...
            {
                case BLOCK -> service.createRelation(data.get(i).srcUserId,data.get(i).targetUserId,"BLOCK","User","User");
                case FOLLOW -> service.createRelation(data.get(i).srcUserId,data.get(i).targetUserId,"FOLLOW","User","User");
                case UNBLOCK -> service.deleteRelation(data.get(i).srcUserId,data.get(i).targetUserId,"BLOCK","User","User");
                case UNFOLLOW -> service.deleteRelation(data.get(i).srcUserId,data.get(i).targetUserId,"FOLLOW","User","User");
                default -> {
                    break;
                }
            }
